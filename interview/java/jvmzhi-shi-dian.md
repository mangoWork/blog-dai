...
- 静态分派
  - 依赖静态类型来定位方法执行版本的分派动作称为静态分派（根据静态参数来定位目标方法），典型的应用为overload。静态分派发生在编译阶段，因此静态分配的动作实际上不是由虚拟机执行的。
- 动态分派：
  - 根据运行期间实际类型来确定方法的引用，典型的应用为override。
...
- Survival区：
  - Survival区和Eden区相同都在Java堆的新生代。Survival有两块区：一块是Survival to区，另外一块是Survival from区。这两个区是相对的，在发生一次Minor GC时，from区会和to区互换。在发生Mimor GC时，Survival to区会把一些存活的对象移至老年代，Eden区和Survival from 区会把一些存活的对象复制进Survival to区，并清除内存。
- 老年代：
...
- 标记整理：
  ...
  - 标记整理法与标记清除法相比，整理出的内存是连续的；而与复制法相比不需要多片内存空间。
  - 缺点：
